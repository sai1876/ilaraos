...
export async function POST(
  ...
  try {
    ...
    await logBusinessEvent({
      event_type: 'ca_returned_to_manager',
    ...
    return NextResponse.json({ ok: true, id, status: 'manager_action_required' });
  } catch (error) {
    console.error('Error returning CA review to manager:', error);
    return NextResponse.json({ detail: 'Failed to return CA review to manager' }, { status: 500 });
  }
}
