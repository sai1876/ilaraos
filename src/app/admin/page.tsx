...
export default function AdminPortalPage() {
    ...
    const unsubscribe = auth.onAuthStateChanged(async (user) => {
      if (user && user.email) {
        try {
          ...
          const validAdminRoles = ['owner', 'admin', 'manager', 'staff'];
  ...
  if (!isAuthenticated) {
    return (
      <div className="h-screen w-full bg-[#FAF7F2] text-[#241A15] flex items-center justify-center font-sans overflow-hidden">
        <div className="flex flex-col items-center gap-4">
          <div className="w-8 h-8 border-2 border-[#9A642C] border-t-transparent rounded-full animate-spin" />
          <p className="font-mono text-xs text-[#9A642C] uppercase tracking-widest animate-pulse">Initializing Owner Session...</p>
        </div>
      </div>
    );
  }
            ...
            <div className="flex justify-between items-center w-full gap-2">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <span className="font-serif text-2xl font-bold text-[#9A642C] leading-none tracking-tight block">Ilara</span>
                  <span className="font-sans text-[10px] uppercase tracking-[0.2em] font-extrabold text-[#9A642C] leading-none mt-1.5 block">Owner</span>
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-[#E8DFD3]">
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-baseline gap-1.5">
              ...
              <span className="font-sans text-[9px] uppercase tracking-widest font-extrabold text-[#9A642C] leading-none">Owner</span>
            </div>
          </div>
